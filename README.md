...
Be sure to zip the lambda function using the command line code ```dotnet lambda package --output-package insert_directory/deploy-package.zip``` where deploy-package is your zip file name.

For our second lambda function, we need to have a process for allowing people to subscribe to the service. To do this, we start a separate lambda project just like we did for the first one.
...
Lambda is a service provided by Amazon that allows you to store functions on a cloud and call them in various ways. In our case, we have two lambda functions working for us. One that will send an sms message at a certain time every day, and another one that waits for an sms from a user to try and subscribe or unsubscribe them. Creating an lambda function on AWS is pretty easy, so I will ommit the steps to do so. Your page should looks something like this when youre done:
...
In the Code Source section, you are going to want to upload the zip file of the appropriate lambda functio we created eariler. The subscription lambda function will be dealing with the API Gateway and the sms sender lambda function will be dealing with EventBridge. After you upload your zip file to the lambda function, you are going to want to make sure you update the runtime settings. Specifically, the default Handler or else you'll run into a time consuming error...
...
Basically, we telling Lambda what function to run when we call this lambda function (much like the main() method in c++). We start off with the namespace of the project, then we tell lambda that we are going to give it a function. Finally, we tell lambda which function we are going to run. In the case of SmsSubscriptionHandler, there is a method called FunctionHandler which is specifically equipped to be run by Lambda in AWS.
...
```
We can use this template to test out any kind of response our subscription service will use based on the body og the text message.
...
After we are sure that our code is working as we want it do, we need to set a trigger. In the case of the subscription lambda function, our trigger is going to be an API Gateway. There is a section about how to set up the API Gateway. Here I will explain how to set it up as a trigger. 

For our lambda function, we want to head to the configuration tab, which is just below the description of the lambda function. From here, we want to click on the "Triggers" tab to the left. Create a new trigger and set the type as an API Gateway. Find the API Gateway that we have created and select is to be our trigger. Now, this subscription lambda function should be triggered by anything that calls our API Gateway endpoint URL.
...
the API Gateway is essentially how you are going to connect your sms messages to your subscription lambda function. When a user texted your Twilio number, a POST response is sent to your APIGateway, which is a URL. This URL then takes the information from the text message and sends it over to your integrated lambda function. In our case, this lambda function will be dealing with subcribing and unsubscribing numbers.
...
To make a stage, we need to implement a "method" for our API Gateway to run. This method is going to be our lambda function that focuses on subscriptions. On the resource page, click on the "Create Method" button.
